...
interface ExtractionMethodsCardProps {
  ...
  updateExtractionData: (data: any) => void;
...
export default function ExtractionMethodsCard({
  selectedEvalMethod,
      ...
      <div className="flex items-center space-x-4">
        ...
        {selectedEvalMethod === 'CROSS-REFINE' && (
          <div className="flex items-center space-x-2">
            ...
            <Select
              value={extractionData.crossRefineModel || ''}
              onValueChange={(value: string) => {
                updateExtractionData({ crossRefineModel: value === "" ? null : (value as ModelOption) });
              }}
            >
              ...
              >
                {modelGroups.map((group) => {
                  const availableModels = group.models.filter(model => 
                    !extractionData.selectedModels.includes(model.value as ModelOption)
                  );
             ...
             getModelProvider(extractionData.crossRefineModel) !== getModelProvider(extractionData.selectedModels[0]) && (
              <div className="flex items-center space-x-2 px-3">
                <label className="text-sm text-slate-300 whitespace-nowra">
                  {(() => {
                    const feedbackModel = extractionData.crossRefineModel;
                    if (['gpt-4o-2024-11-20', 'gpt-4.1-2025-04-14', 'gpt-4.1-nano-2025-04-14'].includes(feedbackModel!)) {
                      return 'OpenAI API Key:';
                    } else if (['claude-sonnet-4-20250514', 'claude-opus-4-20250514'].includes(feedbackModel!)) {
