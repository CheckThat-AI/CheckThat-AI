"""
Evaluation session management for storing extracted claims and reference data.

This service manages the storage of evaluation results so that metrics can be
...
class EvaluationSessionManager:
    """
    Manages evaluation sessions and stores claim data for metric calculations.
    ...
    def __init__(self, max_sessions: int = 1000, session_ttl: int = 3600):
        ...
        """
        self._sessions: Dict[str, EvaluationData] = {}
        self._lock = Lock()
        self.max_sessions = max_sessions
        self.session_ttl = session_ttl
        logger.info(f"EvaluationSessionManager initialized (max_sessions={max_sessions}, ttl={session_ttl}s)")
    ...
    def store_evaluation_data(
        ...
        try:
            with self._lock:
                ...
                self._sessions[session_id] = evaluation_data
                
                logger.info(f"Stored evaluation data for session {session_id}: "
                          f"{len(extracted_claims)} extracted claims, {len(reference_claims)} reference claims")
                return True
                
        except Exception as e:
            logger.error(f"Failed to store evaluation data for session {session_id}: {str(e)}")
            return False
    
    def get_evaluation_data(self, session_id: str) -> Optional[EvaluationData]:
        """
        ...
        Returns:
            EvaluationData if found, None otherwise
        """
        try:
            with self._lock:
                if session_id in self._sessions:
                    eval_data = self._sessions[session_id]
                    eval_data.last_accessed = time.time()  # Update access time
                    return eval_data
                else:
                    logger.warning(f"Session {session_id} not found")
                    return None
        except Exception as e:
            logger.error(f"Failed to retrieve evaluation data for session {session_id}: {str(e)}")
            return None
    ...
    def get_claims_for_metrics(self, session_id: str) -> Optional[tuple[List[str], List[str]]]:
        ...
        """
        eval_data = self.get_evaluation_data(session_id)
        if eval_data:
            return eval_data.extracted_claims, eval_data.reference_claims
        return None
    ...
    def list_sessions(self) -> List[Dict[str, Any]]:
        ...
        try:
            with self._lock:
                session_list = []
                for session_id, eval_data in self._sessions.items():
                    session_info = {
                        "session_id": session_id,
                        ...
                        "age_seconds": time.time() - eval_data.created_at
                    }
    ...
    def _cleanup_expired_sessions(self) -> int:
        ...
        for session_id, eval_data in self._sessions.items():
            if current_time - eval_data.last_accessed > self.session_ttl:
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        try:
            with self._lock:
                ...
                current_time = time.time()
                ages = [current_time - eval_data.created_at for eval_data in self._sessions.values()]
                total_claims = sum(len(eval_data.extracted_claims) for eval_data in self._sessions.values())
                ...
                return {
                    ...
                    "total_stored_claims": total_claims,
                    "average_claims_per_session": total_claims / len(self._sessions) if self._sessions else 0
                }
        except Exception as e:
...
evaluation_session_manager = EvaluationSessionManager() 