...
class BleuConfig(BaseModel):
    ...
    smoothing_function: Optional[str] = "method1"
    ...
    def validate_weights(cls, v):
        if not isinstance(v, (tuple, list)) or len(v) != 4:
            raise ValueError("Weights must be a tuple/list of 4 values")
    ...
    @field_validator('smoothing_function')
    @classmethod
    def validate_smoothing(cls, v):
        if v is not None and v not in ["method0", "method1", "method2", "method3", "method4", "method5", "method6", "method7"]:
...
class BleuResult(BaseModel):
    """Result container for BLEU score calculation"""
    score: float
...
def performance_monitor(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        try:
            ...
            if isinstance(result, BleuResult):
...
class BleuCalculator:
    """
    Professional BLEU score calculator with configuration, caching, and validation.
    ...
    def __init__(self, config: Optional[BleuConfig] = None):
        ...
        Args:
            config: Configuration object for BLEU calculation
        """
    ...
    def _initialize_smoothing(self) -> None:
        ...
        if self.config.smoothing_function:
            smoothing = SmoothingFunction()
            self._smoothing_func = getattr(smoothing, self.config.smoothing_function, None)
            if self._smoothing_func is None:
                logger.warning(f"Smoothing function {self.config.smoothing_function} not found, using default")
        else:
            self._smoothing_func = None
    ...
    def calculate_score(self, response_text: str, reference_text: str, 
                       return_detailed: bool = False) -> Union[float, BleuResult]:
        """
        Calculate BLEU score between response text and reference text.
        ...
        Returns:
            BLEU score (float) or detailed result object
        ...
        try:
            ...
            # Tokenize
            reference_tokens = word_tokenize(str(reference_text))
            ...
            bleu_score = sentence_bleu(
                ...
                smoothing_function=self._smoothing_func,
                auto_reweigh=self.config.auto_reweigh
            )
            
            # Cache result
            if self.config.cache_enabled and cache_key:
                self._cache[cache_key] = bleu_score
                self._manage_cache()
            ...
            raise ValueError(f"Failed to calculate BLEU score: {str(e)}")
    
    @validate_inputs
    @performance_monitor
    def calculate_batch(self, response_texts: List[str], reference_texts: List[str],
                       return_detailed: bool = False) -> Union[List[float], List[BleuResult]]:
        """
        Calculate BLEU scores for a batch of text pairs.
        ...
        Returns:
            List of BLEU scores or detailed result objects
        ...
        for i, (response, reference) in enumerate(zip(response_texts, reference_texts)):
            try:
                ...
                if return_detailed:
                    results.append(BleuResult(score=0.0, execution_time=0.0, 
    ...
    def update_config(self, **kwargs) -> None:
        ...
        self.config = BleuConfig(**config_dict)
        
        # Reinitialize smoothing if function changed
...
def calculate_bleu_score(response_text: str, reference_text: str) -> float:
    """
    Calculate BLEU score between response text and reference text.
...
def calculate_bleu_score_batch(response_texts: List[str], reference_texts: List[str]) -> List[float]:
    """
    Calculate BLEU scores for a batch of text pairs.
